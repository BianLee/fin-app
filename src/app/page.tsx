...
const Home = () => {
  ...
  // Financial market flow data
  const financialFlowData = {
    direct: "Lenders provide funds directly to borrowers through financial markets (stocks, bonds)",
    indirect: "Lenders provide funds to financial intermediaries (banks, credit unions) who then lend to borrowers"
  };
  ...
  return (
    <div className="p-4 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-center gap-4 mb-6">
        <button 
          className={`px-4 py-2 rounded ${activeTab === 'bonds' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          onClick={() => setActiveTab('bonds')}
        >
          Bond Market Model
        </button>
        <button 
          className={`px-4 py-2 rounded ${activeTab === 'money' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          onClick={() => setActiveTab('money')}
        >
          Money & Interest Rates
        </button>
        <button 
          className={`px-4 py-2 rounded ${activeTab === 'financial' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          onClick={() => setActiveTab('financial')}
        >
          Financial System
        </button>
        <button 
          className={`px-4 py-2 rounded ${activeTab === 'fisher' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          onClick={() => setActiveTab('fisher')}
        >
          Fisher Effect
        </button>
      </div>
